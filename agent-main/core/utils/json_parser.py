...
class RobustJSONParser:
    ...
    def fix_common_errors(text: str) -> str:
        ...
        # 2. 修复截断的JSON字符串（智能补全）
        text = RobustJSONParser.fix_truncated_json(text)

        # 3. 修复单引号为双引号 (注意: 这可能会误伤字符串内容,需谨慎)
        # text = text.replace("'", '"')

        # 4. 确保键名有引号 (简化处理,可能不完美)
    ...
    @staticmethod
    def extract_valid_json_prefix(text: str) -> Optional[str]:
        """从可能损坏的JSON中提取有效的JSON前缀

        当JSON被截断或损坏时，尝试提取最长的有效JSON部分
        ...
        Returns:
            有效的JSON字符串，或None
        """
        # 尝试逐步缩短文本，找到第一个能成功解析的部分
        # 从完整文本开始，逐步减少
        lines = text.split('\n')
        valid_text = ""

        for i, line in enumerate(lines):
            test_text = valid_text + line + '\n'
            try:
                # 尝试解析
                json.loads(test_text)
                valid_text = test_text
            except:
                # 如果这一行导致解析失败，尝试在行内逐步回退
                line_stripped = line.rstrip()
                for j in range(len(line_stripped) - 1, -1, -1):
                    test_text = valid_text + line_stripped[:j]
                    try:
                        json.loads(test_text)
                        valid_text = test_text
                        break
        ...
        if valid_text:
            logger.info(f"✅ 提取到有效JSON前缀，长度: {len(valid_text)} 字符")
            return valid_text
    ...
    def fix_truncated_json(text: str) -> str:
        ...
        if in_string:
            text += '"'
            logger.info("🔧 修复截断的JSON: 添加闭合引号")

        # 补全缺失的括号（按照正确的顺序）
        # 先补全 ]
        for _ in range(open_brackets - close_brackets):
            text += ']'
            logger.info("🔧 修复截断的JSON: 添加闭合 ]")
        ...
        # 尝试修复最后未完成的项（如 "key": value 未闭合）
        text = text.rstrip()
        if text and text[-1] in ',:':
            # 移除末尾的逗号或冒号
            text = text[:-1]
            logger.info("🔧 修复截断的JSON: 移除末尾逗号/冒号")

        # 检查是否是未完成的字符串值
        if text and text[-1] == ':' and len(text.split(':')) > 1:
            # 填充空字符串
            text += '""'
            logger.info("🔧 修复截断的JSON: 填充空字符串")
    ...
    def parse(cls, text: str) -> Optional[Any]:
        ...
        try:
            ...
            # 步骤7: 尝试提取有效的JSON前缀（作为最后的后备方案）
            logger.info("💡 尝试提取有效的JSON前缀...")
            valid_prefix = cls.extract_valid_json_prefix(json_text)
            if valid_prefix:
                try:
                    result = json.loads(valid_prefix)
                    logger.info(f"✅ 使用JSON前缀解析成功,类型: {type(result).__name__}")
                    return result
                except Exception as prefix_error:
                    logger.error(f"JSON前缀解析也失败: {prefix_error}")
